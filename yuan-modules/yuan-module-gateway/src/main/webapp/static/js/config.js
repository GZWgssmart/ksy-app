    ...
    var kingAdminName = 'yuan-module-admin';
    var kingInterfaceName = 'yuan-module-interface';
    var kingGateway = 'yuan-module-gateway';
    
	window.isLocalhost = window.location.hostname == "localhost";
	window.ctx = "";
	if (window.location.protocol == "file:") {
	    var url = window.location.href.split("/yuan/");
	    if (url.length == 2) {
	        window.ctx = url[0] + "/";
	    }
	}else {
	    window.ctx = window.location.protocol + "//" + window.location.host + "/"+kingGateway+"/";
	}
	
	window.root = window.ctx + "yuan/";
	...
	window.serverPath = window.server + kingInterfaceName+"/";
	//后台上传的图片地址
	window.serverUploadPath = window.server + kingAdminName;
